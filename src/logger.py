...
try:
    from scapy.all import IP, UDP, PcapWriter as ScapyPcapWriter
except ImportError:
    ScapyPcapWriter = None
...
class PcapWriter:
    """AÄ paketlerini PCAP dosyasÄ±na kaydeden sÄ±nÄ±f"""
    def __init__(self, filepath: str):
        self.filepath = filepath
        self.writer = None
        if ScapyPcapWriter:
            self.writer = ScapyPcapWriter(self.filepath, append=True, sync=True)
            
    def write_packet(self, src_ip: str, dst_ip: str, src_port: int, dst_port: int, payload: bytes):
        if self.writer:
