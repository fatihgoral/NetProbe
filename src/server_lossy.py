...
class LossyNetProbeServer:
    ...
    def __init__(self, host: str = "0.0.0.0", port: int = 5002,
        ...
        print(f"✓ Lossy Server: kayıp oranı = %{loss_rate*100:.0f}")
    ...
    def start(self):
        ...
        self.running = True
        print(f"✓ Lossy Sunucu başlatıldı: {self.host}:{self.port}")
    ...
    def _handle_end_packet(self, data: bytes, client_addr: tuple):
        ...
        try:
            ...
            if file_hash == self.file_hash:
                ...
                print(f"⚠ Hash uyuşmazlığı (kayıp paketler nedeniyle beklenen)")
            
            success_rate = (len(self.received_packets) / total_packets * 100) if total_packets > 0 else 0
            print(f"✓ Transfer tamamlandı: {len(self.received_packets)}/{total_packets} paket (%{success_rate:.2f})")
