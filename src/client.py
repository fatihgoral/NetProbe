...
class NetProbeClient:
    ...
    def transfer_file(self, filepath: str) -> bool:
        ...
        try:
            ...
            # 9. Ana Gönderim Döngüsü — Selective Repeat Sliding Window
            # window_base: onay bekleyen en küçük seq numarası (pencerenin sol kenarı)
            # next_seq_num: bir sonraki gönderilecek yeni paketin numarası
            # unacked_packets: {seq: (payload, gönderim_zamanı, retry_sayısı)}
            next_seq_num = 0

            while self.window_base < total_packets and self.is_transferring:
                with self.lock:
                    # --- Yeni Paket Gönderimi ---
                    # Pencere dolmadığı sürece (next_seq < base + window_size) yeni paket gönder.
                    # Bu, birden fazla paketin ACK beklenmeksizin arka arkaya iletilmesini sağlar.
                    while next_seq_num < self.window_base + self.window_size and next_seq_num < total_packets:
                        ...
                        # Gönderim zamanını kaydet; timeout hesabında kullanılır
                        self.unacked_packets[next_seq_num] = (payload, time.time(), 0)
                        next_seq_num += 1

                    # --- Timeout Kontrolü ve Yeniden İletim ---
                    # Her iteration'da onay bekleyen paketlerin üzerinden geç.
                    # Selective Repeat: sadece zaman aşımına uğrayan paket tekrar gönderilir,
                    # Go-Back-N'den farklı olarak tüm pencere geri sarılmaz.
                    current_time = time.time()
                    for seq, (payload, send_time, retries) in list(self.unacked_packets.items()):
                        # Her retry'da timeout biraz artar (adaptif bekleme)
                        current_timeout = self.timeout + retries

                        if current_time - send_time > current_timeout:
                            if retries >= self.max_retries:
                                # Maksimum deneme aşıldı → paketi başarısız say, log'a yaz
                                ...
                                # window_base bu paket ise pencereyi ilerlet,
                                # aksi hâlde hiçbir zaman ilerlemez (deadlock).
                                if seq == self.window_base:
                                    self._advance_window(total_packets)
                            else:
                                # Timeout doldu ama retry hakkı var → paketi yeniden gönder
