...
class NetProbeClient:
    ...
    def transfer_file(self, filepath: str) -> bool:
        ...
        print(f"✓ Dosya: {filepath.name} ({file_size:,} bytes)")
        ...
        try:
            ...
            print(f"✓ Dosya bölündü: {total_packets} paket ({self.packet_size} bytes/paket)")
            ...
            if failed_packets:
                print(f"\n✗ Başarısız paketler: {failed_packets}")
    ...
    def _send_start_packet(self, total_packets: int, file_hash: bytes):
        ...
        print(f"✓ START paketi gönderildi ({total_packets} paket)")
