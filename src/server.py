...
class NetProbeServer:
    ...
    def _handle_data_packet(self, data: bytes, session: ClientSession):
        ...
        try:
            ...
            # Checksum doğrulaması başarısız → NACK ile hızlı yeniden iletimi tetikle
        ...
        if seq_num in session.received_packets:
            # Duplicate paket: zaten alınmış, dosyaya ikinci kez yazılmaz.
            # Föy gereği: alıcı uygun ACK'i tekrar gönderir ve paketi yok sayar.
            self.logger.log_event(
                EventType.DUPLICATE,
                seq_num=seq_num,
                status="WARNING",
                details=f"Duplicate paket alındı: {seq_num}, yoksayılıyor"
            )
            print(f"[WARN] Duplicate paket yoksayıldı: {seq_num}")
        else:
            # İlk kez gelen paket → buffer'a ekle
            session.received_packets[seq_num] = payload
            print(f"[OK] Paket alındı: {seq_num}/{session.total_expected_packets}")

        # Her iki durumda da ACK gönder (duplicate'de de gönderilmeli)
