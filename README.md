...
> **Not:** PCAP kaydı (`--pcap`) için Windows'ta [Npcap](https://npcap.com/#download) yüklenmelidir. PCAP kullanılmadan sistem sorunsuz çalışır.

---

### Çalıştırma Adımları

Tüm komutlar `src/` klasöründen çalıştırılmalıdır:
...
```bash
python server.py --host 0.0.0.0 --port 5000 --output-dir ../received_files
```

| Parametre | Açıklama | Varsayılan |
|---|---|---|
| `--host` | Dinlenecek ağ arayüzü | `0.0.0.0` |
| `--port` | UDP port numarası | `5000` |
| `--output-dir` | Alınan dosyaların kaydedileceği klasör | `received_files` |
| `--pcap` | Wireshark için PCAP dosya yolu (opsiyonel) | — |

#### 2. Dosya Gönder (Terminal 2)

```bash
python client.py --file ../data/large_test.bin --host 127.0.0.1 --port 5000
```

| Parametre | Açıklama | Varsayılan |
|---|---|---|
| `--file` | Gönderilecek dosya yolu | (zorunlu) |
| `--host` | Sunucu IP adresi | `localhost` |
| `--port` | Sunucu port numarası | `5000` |
| `--packet-size` | Paket boyutu (byte) | `1024` |
| `--timeout` | ACK bekleme süresi (saniye) | `2.0` |
| `--max-retries` | Maksimum yeniden gönderim sayısı | `5` |
| `--window-size` | Sliding window boyutu | `10` |
| `--compress` | Zlib sıkıştırmayı etkinleştir | kapalı |
| `--encrypt` | XOR şifrelemeyi etkinleştir | kapalı |
| `--pcap` | Wireshark için PCAP dosya yolu | — |

#### 3. Deney Senaryolarını Çalıştır
...
**Paket kaybı simülasyonu ile transfer:**
```bash
# Terminal 1 — kayıplı sunucu
python server_lossy.py --loss-rate 0.1 --port 5000

# Terminal 2 — istemci
python client.py --file ../data/large_test.bin --timeout 0.5
```

**Sıkıştırma + şifreleme ile transfer:**
```bash
python client.py --file ../data/large_test.bin --compress --encrypt
```

**PCAP kaydı ile transfer (Npcap gerektirir):**
```bash
python server.py --pcap ../server_trafigi.pcap
python client.py --file ../data/large_test.bin --pcap ../client_trafigi.pcap
```
...
NetProbe/
├── src/                        # Kaynak kodlar
│   ├── protocol.py             # Paket yapıları, serileştirme, checksum (SHA-256)
│   ├── client.py               # UDP istemci — Selective Repeat Sliding Window
│   ├── server.py               # UDP sunucu — çoklu istemci, duplicate tespiti
│   ├── server_lossy.py         # Yapay paket kaybı simülasyonlu sunucu
│   ├── loss_simulator.py       # Kayıp ve gecikme simülatörü
│   ├── logger.py               # CSV loglama + PCAP trafik kaydı
│   ├── metrics.py              # Throughput, Goodput, RTT hesaplama
│   ├── dashboard.py            # rich tabanlı gerçek zamanlı izleme paneli
│   └── __init__.py
├── experiments/                # Deney senaryoları
│   ├── scenario1_packetsize.py
│   ├── scenario2_timeout.py
│   ├── scenario3_loss.py
│   ├── scenario4_filesize.py
│   ├── scenario5_tcp_comparison.py
│   └── scenario_utils.py
├── analysis/
│   └── plots.py                # Grafik üretim scripti (reports/ klasörüne yazar)
├── data/                       # Deney veri dosyaları ve CSV sonuçlar
│   ├── scenario1_results.csv
│   ├── scenario2_results.csv
│   ├── scenario3_results.csv
│   ├── scenario4_results.csv
│   └── large_test.bin
├── reports/                    # Üretilen grafikler
│   ├── scenario1_paket_boyutu.png
│   ├── scenario2_timeout.png
│   ├── scenario3_paket_kaybi.png
│   ├── scenario4_dosya_boyutu.png
│   ├── netprobe_dashboard.png
│   └── graphics/               # Detaylı 4'lü panel grafikler
├── tests/
│   ├── conftest.py             # pytest yapılandırması
│   └── test_protocol.py        # 27 birim testi
├── received_files/             # Sunucunun aldığı dosyalar
├── generate_report_graphics.py # Rapor için detaylı grafik üretimi
├── run_all_scenarios.py        # Tüm deneyleri sırayla çalıştırır
├── requirements.txt
└── README.md
```
...
- Duplicate paket tespiti ve yoksayma (föy zorunlu gereksinimi)
...
│ Paket[i+1] Gönder ──────┼───────────────────▶     │
│ ...                     │                         ▼
│ Paket[i+N-1] Gönder ────┼───────────────────▶  Duplicate? → ACK gönder, yoksay
│                         │                         │
...
- Goodput **%99,7 düşmüştür** (13.158 → 44 Kbps)
- Tamamlanma süresi **~306 kat artmıştır** (0,16s → 48,97s)
...
> **Sliding Window mekanizmasının kritik önemi:** Stop-and-Wait'te boşta geçen bekleme süresi, Selective Repeat ile ortadan kaldırılarak aynı kayıp oranlarında çok daha yüksek throughput elde edilebilir.
...
2026-06-07T23:49:45.700,DUPLICATE,5,0,WARNING,Duplicate paket alindi: 5, yoksayiliyor
```
...
1. **Paket boyutu, throughput'u doğrudan etkiler.** Başlık overhead'inin toplam paket içindeki oranı düştükçe verimlilik artar. Ancak MTU sınırı nedeniyle 1024B optimal değerdir.
