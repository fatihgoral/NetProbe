...
# Grafik 3.4: Gerçek Paket Kaybı Oranı
ax = axes[1, 1]
ax.bar(df3['loss_rate_pct'].astype(int).astype(str) + '%', df3['actual_packet_loss_pct'], 
       color='#95E1D3', alpha=0.8, edgecolor='black')
ax.set_xlabel('Simüle Edilen Kaybı Oranı', fontweight='bold')
ax.set_ylabel('Gerçek Kaybı Oranı (%)', fontweight='bold')
ax.set_title('Simüle Edilen vs Gerçek Paket Kaybı')
ax.grid(axis='y', alpha=0.3)
